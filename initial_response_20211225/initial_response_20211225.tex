...
\rcomment{
Page 6 row 43 right. 
Here you use $M_0$, while in start of Figure 6 $M_s$ is used.
}
\textbf{Response.} 
...
\textbf{Response.}
We have added an explanation of ``free area'' 
in the beginning of section Methodology.

\rcomment{
Figure 6. In the caption you write that this is a process of 
``merging events for a step''. 
In the bottom of figure 5 it is stated ``all the steps''.
}
\textbf{Response.}
Figure 6 indeed presents the merging events for a single step,
while Figure 5 finds merging events for all the steps.
We made this clearer in the section of a greedy algorithm.
...
\textbf{Response.}
No, we have removed the value of the simultaneous parameter.
...
\textbf{Response.}
We have added a comment of the relationship.

\rcomment{
Equation 2: Both $s$ and ``$s_\mathrm{high}$'' 
are input to the function. 
Not only ``$s_\mathrm{high}$''.
}
\textbf{Response.}
Both ``$s_\mathrm{high}$'' and ``$S_\mathrm{high}$'' are inputs.
We have added ``$S_\mathrm{high}$'' as an input.


\rcomment{
Table 1b: Is the value of $f_\mathrm{id}$ 10 really correct? 
Could $s_\mathrm{merge}$  be lower than $s_\mathrm{low}$
}
\textbf{Response.}
No, the value was wrong. 
...
\textbf{Response.}
Yes, Figure 8 is referred to 
in the section of animation duration of a step.
