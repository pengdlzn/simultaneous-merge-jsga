...
In order to ensure that every change has more time to take place, 
...
and we discuss the animation time gained for each generalization operation.
...
\todo[inline]{Would be nice to include some illustrations from related work,
may be best 1 or 2 illustrations from our own earlier work. 
add eater and hierarchies of classes.
}
...
\todo[inline]{May be include reference to PhD thesis of John van Smaalen;
class driven generalization; in fig 22 and the Class adjacency index and the
object aggregation factor; see sec. 4.5, 4.6 in the PhD thesis:
J.W.N. van Smaalen, Automated Aggregation of Geographic Objects,
A New Approach to the Conceptual Generalisation of Geographic Databases,
Wageningen Universiteit, 18 november 2003
}
...
We performed this multiplication for illustrative purpose only, so that the contents of the SSC in the figures can be better observed.
Note that the merging operation is applied in a pre-processing step, before the user zooming in and out of the map and is thus independent of users' area objects of interest.
...
%where smooth and simultaneous changes going on can be found in figure~(r);
%digital map with smooth and simultaneous generalization operations.
%
...
%there is a less important area in each of the simultaneous events of a merging step.
...
\end{figure*}
\todo[inline]{\emph{Move this note to online interface:}
